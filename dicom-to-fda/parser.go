...
// MergeMetadata overrides DICOM patient fields with non-empty values from m.
func (d *DicomData) MergeMetadata(m *PatientMetadata) {
	if m.PatientName != "" {
		d.Patient.PatientName = m.PatientName
	}
	if m.PatientID != "" {
		d.Patient.PatientID = m.PatientID
	}
	if m.SecondPatientID != "" {
		d.Patient.SecondPatientID = m.SecondPatientID
	}
	if m.PatientBirthDate != "" {
		d.Patient.PatientBirthDate = m.PatientBirthDate
	}
	if m.PatientSex != "" {
		d.Patient.PatientSex = m.PatientSex
	}
	if m.PatientAge != "" {
		d.Patient.PatientAge = m.PatientAge
	}
	if m.Paced != "" {
		d.Patient.Paced = m.Paced
	}
	if m.Bed != "" {
		d.Patient.Bed = m.Bed
	}
	if m.Room != "" {
		d.Patient.Room = m.Room
	}
	if m.PointOfCare != "" {
		d.Patient.PointOfCare = m.PointOfCare
	}
	if len(m.Medications) > 0 {
		d.Patient.Medications = m.Medications
	}
	if len(m.ClinicalClassifications) > 0 {
		d.Patient.ClinicalClassifications = m.ClinicalClassifications
	}
	if m.LocationName != "" {
		d.Patient.LocationName = m.LocationName
	}
	if m.InvestigatorName != "" {
		d.Patient.InvestigatorName = m.InvestigatorName
	}
	if m.DeviceModel != "" {
		d.Patient.DeviceModel = m.DeviceModel
	}
	if m.DeviceSerial != "" {
		d.Patient.DeviceSerial = m.DeviceSerial
	}
	if m.SoftwareVersion != "" {
		d.Patient.SoftwareVersion = m.SoftwareVersion
	}
	if m.Manufacturer != "" {
		d.Patient.Manufacturer = m.Manufacturer
	}
	if m.OperatorsName != "" {
		d.Patient.OperatorsName = m.OperatorsName
	}
	if m.InstitutionName != "" {
		d.Patient.InstitutionName = m.InstitutionName
	}
	if m.StudyDate != "" {
		d.StudyDate = m.StudyDate
	}
	if m.StudyTime != "" {
		d.StudyTime = m.StudyTime
	}
}
...
func parseAnnotation(elements []*dicom.Element) WaveformAnnotation {
	...
	// MeasuredValueSequence → numeric value + unit
	if e := findElement(elements, tag.MeasuredValueSequence); e != nil {
		if items, ok := e.Value.GetValue().([]*dicom.SequenceItemValue); ok && len(items) > 0 {
			...
			// MeasurementUnitsCodeSequence → unit CodeMeaning
